...
@php($pages = \App\Models\Page::select(['title','slug'])->active()->get())
                    ...
                    <div class="modal-body">
                        ...
                        <div id="link_selector"
                            ...
                            <select id="m_link_type" class="form-control" name="type">
                                ...
                                @if(auth()->id() == 1) <option value="route">Dynamic Route</option> @endif
                            ...
                            <div id="m_page_type">
                                <label for="m_page">Select Page</label>
                                <select id="m_page" class="form-control">
                        ...
                        <br>
                        <label class="control-label" for="m_status">Status</label>
...
@section('javascript')
    ...
    <script>
        $(document).ready(function () {
            ...
            $m_page.on('change', function (e) {
                $m_url.val($m_page.val())
            });

            function linkSelector(selector = null)
            {
                switch (selector)  {
                    case 'route':
                        $m_route_type.show()
            ...
            $m_modal.on('show.bs.modal', function(e, data) {
                ...
                if (_adding) {
                    ...
                    $m_status.val(_src.data('status')).change();

                    let pages = @json(collect($pages)->map(function ($page) { return "/$page->slug"; }))
                    ...
                    if (_src.data('route') != "") {
                        ...
                        $('#link_selector').show()
                        if(pages.includes(_src.data('url'))){
                            $m_link_type.val('page').change();
                            $m_page.val(_src.data('url')).change();
                        } else{
                            $m_link_type.val('url').change();
